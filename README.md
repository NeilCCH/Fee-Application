...
`frontend/manifest.json` + `icon-192.png`／`icon-512.png` 讓手機瀏覽器（iOS Safari「加入主畫面」、Android Chrome「安裝應用程式」）跟桌機瀏覽器網址列都會出現安裝選項，開啟後會像 App 一樣全螢幕、有自己的圖示，不會看到瀏覽器網址列。圖示沿用 `favicon.png`（皇冠 GO 圖示）放大／縮小產生。

## 表單頁的費用類別配色

表單頁（`frontend/`）用顏色區分費用類別，方便一眼辨識：交通＝藍、住宿／膳雜＝青、交際費＝紫、文康費＝粉、雜支／其他＝橘／灰、出差彙總＝深藍（品牌色）。套用在出差明細的金額分組、費用類別選擇按鈕、草稿清單列、預覽明細的圓點上，配色定義集中在 `frontend/style.css` 的 `.cat-*` class（`--cat`/`--cat-bg` 兩個 CSS 變數）。
